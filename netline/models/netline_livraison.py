    ...
    def create_invoice(self):
        ...
        for l in nll:
            ...
            if len(l.reception_line_id.product_id) >0:
                cps_facturation_lines.append({ 'product_id': l.reception_line_id.product_id.product_id.id, 'product_name': l.reception_line_id.product_id.name, 'product_description' : l.reception_line_id.product_id.name, 'sequence': i, 'qty_to_invoice': l.to_deliver_quantity, 'quantity_livre': l.to_deliver_quantity, 'reste_a_facturer': l.to_deliver_quantity, 'price' : l.reception_line_id.product_id.prix_lavage, 'departement':'', 'fonction':''})
            if len(l.reception_line_id.product_pressing_id) >0:
                cps_facturation_lines.append({ 'product_id': l.reception_line_id.product_pressing_id.product_id.id, 'product_name': l.reception_line_id.product_pressing_id.name,  'product_description' : l.reception_line_id.product_pressing_id.name, 'sequence': i, 'qty_to_invoice': l.to_deliver_quantity, 'quantity_livre': l.to_deliver_quantity, 'reste_a_facturer': l.to_deliver_quantity, 'price' : l.reception_line_id.product_pressing_id.prix_lavage, 'departement':'', 'fonction':''})
            if len(l.reception_line_id.product_vt_id) >0:
                cps_facturation_lines.append({ 'product_id': l.reception_line_id.product_vt_id.product_id.id, 'product_name': l.reception_line_id.product_vt_id.name, 'product_description' : l.reception_line_id.product_vt_id.name, 'sequence': i, 'qty_to_invoice': l.to_deliver_quantity, 'quantity_livre': l.to_deliver_quantity, 'reste_a_facturer': l.to_deliver_quantity, 'price' : l.reception_line_id.product_vt_id.prix_lavage, 'departement': l.reception_line_id.product_vt_id.departement_id.name, 'fonction': l.reception_line_id.product_vt_id.fonction_id.name })
        ...
        for netline_facturation_line in cps_facturation_lines:
            line_index = str(netline_facturation_line['product_id'])
            if netline_facturation_line['product_name'] is not False:
                line_index += netline_facturation_line['product_name']
